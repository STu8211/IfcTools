...
def export_ifc_to_csvs(ifc_path, output_dir):
            ...
            if hasattr(elemento, "HasAssociations"):
                for assoc in elemento.HasAssociations:
                    if assoc.is_a("IfcRelAssociatesMaterial"):
                        # Extract material from the association
    ...
    # For each element, extract quantity sets and their values (length, area, volume, etc.)
    with open(f"{output_dir}/quantities.csv", mode='w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
    ...
    with open(f"{output_dir}/entities.csv", mode='w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["GlobalID", "Name", "IfcEntity", "PredefinedType", "TypeName", "ObjectType"])
        for elemento in ifc.by_type('IfcElement'):
    ...
    #use information from the header
        file_schema = header_info["schema_name"]  # Example: IFC4X3_ADD2
    ...
    print(f"✅ Información general del archivo IFC exportada correctamente en: {output_dir}")

    # --- STEP 6: Export Floors and Elements ---
    # For each floor (building storey), export its elements and their basic info
    with open(f"{output_dir}/floors_and_elements.csv", mode='w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
    ...
    print(f"✅ Información de pisos y elementos exportada correctamente en: {output_dir}")

if __name__ == "__main__":
    # Example usage: specify IFC file path and output directory
    export_ifc_to_csvs(r'C:\Users\FlorenciaGomez\SIDE\SIDE - Team\17_Severin\05_KI\IFC Models for Training\_IFC-Modelle nach Autorensoftware\Trimble NOVA_00816_033_FM_GT_UK01.ifc' , r'C:\Users\FlorenciaGomez\Desktop\SIDE_APP\Data_to_PBI')
